...
const CompStatistics = (props) => {
    const {projectId,complexityStat}=props
    ...
    useEffect(async() => {
        await getComplexity(findNum)
    }, []);


    //复杂度查询
    const getComplexity = (findNum) => {
        complexityStat({projectId:projectId,
            findNum:findNum
        }).then(res=>{
